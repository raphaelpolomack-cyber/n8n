...
\begin{enumerate}[label=(\roman*)]
    ...
    Le premier terme, $\iint_S \vect{j} \cdot \mathrm{d}\vect{S}$, représente le courant de conduction $I_{conduction}$ traversant la surface $S$ et \text{enlacé} par le contour $\Gamma$.
...
\begin{itemize}[label=$\times$]
    \item \textbf{Confusion entre le théorème d'Ampère statique et généralisé :} Le théorème d'Ampère simple ($\oint_{\Gamma} \vect{B} \cdot \mathrm{d}\vect{l} = \mu_0 I_{\text{enlacé}}$) n'est valide qu'en régime stationnaire. Il est crucial d'inclure le terme de courant de déplacement ($\mu_0 \varepsilon_0 \frac{\mathrm{d}\Phi_E}{\mathrm{d}t}$) en régime variable.
