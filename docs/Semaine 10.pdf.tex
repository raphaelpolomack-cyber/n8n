...
\begin{itemize}
    \item \textbf{Accents dans les indices mathématiques} : Oubli de supprimer les accents (ex: $I_{\text{enlace}}$ au lieu de $I_{enlacé}$).
    \item \textbf{Syntaxe \textbf{}} : Oubli de l'accolade ouvrante ou fermante après `\textbf` (ex: `\textbfLe texte` au lieu de `\textbf{Le texte}`).
