...
Le théorème de Gauss est une formulation intégrale de l'équation de Maxwell-Gauss. Il relie le flux du champ électrique $\vec{E}$ à travers une surface fermée à la charge électrique totale contenue dans le volume délimité par cette surface.
...
\begin{enumerate}[(a)]
    ...
    \begin{equation*}
        \oint_{\Gamma} \vec{B} \cdot \dd \vec{l}
        = \oint_{\Gamma} B(r) \vec{u}_{\theta} \cdot (r \dd\theta \vec{u}_{\theta})
        = B(r) \int_0^{2\pi} r \dd\theta
        = B(r) (2\pi r)
    \end{equation*}
...
Considérons un cylindre plein infini de rayon $R$, orienté selon l'axe $Oz$, parcouru par un courant total $I$ uniformément réparti sur sa section. La densité de courant volumique est donc
$j = \frac{I}{\pi R^2}$ pour $r \le R$, et nulle ailleurs.
...
\begin{enumerate}[(a)]
    ...
    \begin{itemize}
        ...
        $I_{\text{enlacé}} = J \times \text{aire du disque de rayon } r
...
\begin{enumerate}[(a)]
    ...
    \[
    \oint_{\Gamma} \vec{B} \cdot \dd \vec{l}
    = \int_A^B \vec{B} \cdot \dd \vec{l}
    + \int_B^C \vec{B} \cdot \dd \vec{l}
    + \int_C^D \vec{B} \cdot \dd \vec{l}
    + \int_D^A \vec{B} \cdot \dd \vec{l}
    \]
    \begin{itemize}
